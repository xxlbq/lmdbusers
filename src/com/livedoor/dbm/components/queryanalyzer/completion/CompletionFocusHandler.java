/**
 * $Id: CompletionFocusHandler.java,v 1.1 2006/10/17 01:10:35 lijian Exp $
 * SQL编辑器帮助
 */
...
/**
 * This tries to handle the focus of the completion popup and tell its listener
 * when the completion popup lost focus. <p/> Note: this class does not send
 * focus gained events.
 */
public class CompletionFocusHandler {
	...
	public CompletionFocusHandler(TextComponentProvider txtComp,
		...
		if (_txtComp.editorEqualsFilter()) {
			_txtComp.getEditor().addFocusListener(new FocusListener() {
				public void focusGained(FocusEvent e) {
					// System.out.println("################editor.focusGained "
					// + _txtComp.getEditor());
					onFocusGained(e);
				}

				public void focusLost(FocusEvent e) {
					// System.out.println("################editor.focusLost " +
					// _txtComp.getEditor());
					onFocusLost(e);
				}
			});
	...
	/**
	 * This method has nothing to do with the focus handling outside the popup
	 * it is needed to keep the focus on the filter, if the popup has its own
	 * filter, for example the tools popup.
	 */
