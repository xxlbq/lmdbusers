 ...
 * @version 1.0
 */
public class OraServerInfoPanel extends ServerInfoPanel implements IConnectionModel {
	...
	private JLabel jLabel_connectAs = null;

	private JComboBox jComboBox_connectAs = null;
	...
	private void initialize() {
		jLabel_connectAs = new JLabel();
		jLabel_connectAs.setBounds(new Rectangle(29, 249, 94, 18));
		jLabel_connectAs.setFont(new Font("Dialog", Font.BOLD, 14));
		jLabel_connectAs.setText("Connect As:");
		...
		this.add(jLabel_connectAs, null);
		this.add(getJComboBox_connectAs(), null);
	}
	 ...
	 *            <p>
	 * 
	 * @return jComboBox_connectAs
	 *         <p>
	 */
	private JComboBox getJComboBox_connectAs() {
		if (jComboBox_connectAs == null) {
			Vector data = new Vector();
			data.add("normal");
			data.add("sysoper");
			data.add("sysdba");
			jComboBox_connectAs = new JComboBox(data);
			jComboBox_connectAs.setBounds(new Rectangle(173, 248, 116, 21));
		}
		return jComboBox_connectAs;
	}
	...
	public ConnectionInfo getConnectionInfo() {
		OraConnectionInfo connectionInfo = new OraConnectionInfo();
		connectionInfo.setConnectionName(jTextField_name.getText());
		connectionInfo.setDbType(DBServerType.ORACLE9i);
		...
		connectionInfo.setRole((String) jComboBox_connectAs.getSelectedItem());
		connectionInfo.setSid(jTextField_dataBase.getText());
	...
	public void loadConnectionInfo(ConnectionInfo info, boolean readOnly) {
		...
		OraConnectionInfo conn = (OraConnectionInfo) info;
		if (readOnly) {
		...
		jTextField_dataBase.setText(conn.getSid());
		jComboBox_connectAs.setSelectedItem(conn.getRole());
	}
}
