/**
 * $Id: PopupManager.java,v 1.1 2006/10/17 01:10:35 lijian Exp $
 * SQL编辑器帮助
 */
...
/**
 * <p>Title: 弹出窗口管理器</p> 
 * <p>Description: 实现了Popup的初始化，启动，设置大小，出现位置等功能</p> 
...
public class PopupManager {
	...
	/**
	 * Place popup to larger area. i.e. if place below cursor is larger than
	 * place above, then popup will be placed below cursor.
	 */
	public static final Placement Largest = new Placement("Largest"); // NOI18N

	/**
	 * Place popup above cursor. If a place above cursor is insufficient, then
	 * popup will be placed below cursor.
	 */
	public static final Placement AbovePreferred = new Placement(
			"AbovePreferred");

	/**
	 * Place popup below cursor. If a place below cursor is insufficient, then
	 * popup will be placed above cursor.
	 */
	public static final Placement BelowPreferred = new Placement(
			"BelowPreferred");
	...
	/**
	 * Install popup component to textComponent root pane based on caret
	 * coordinates with the <CODE>Largest</CODE> placement.
	 * 
	 * @param popup
	 *            popup component to be installed into root pane of the text
	 *            component.
	 */
	...
	public void install(JComponent popup, Rectangle cursorBounds,
		 ...
		 * 
		 * Uninstall the old popup from root pane and install the new one. Even
		 * in case they are the same objects it's necessary to cover the
		 * workspace switches etc.
		 */
		if (this.popup != null) {
	...
	/**
	 * Variation of the method for computing the bounds for the concrete view
	 * component. As the component can possibly be placed in a scroll pane it's
	 * first necessary to translate the cursor bounds and also translate back
	 * the resulting popup bounds.
	 * 
	 * @param popup
	 *            popup panel to be displayed
	 * @param view
	 *            component over which the popup is displayed.
	 * @param cursorBounds
	 *            the bounds of the caret or mouse cursor relative to the
	 *            upper-left corner of the visible view.
	 * @param placement
	 *            where to place the popup panel according to the cursor
	 *            position.
	 * @return bounds of popup panel relative to the upper-left corner of the
	 *         underlying view component. <CODE>null</CODE> if there is no
	 *         place to display popup.
	 */
	protected static Rectangle computeBounds(JComponent popup, JComponent view,
			Rectangle cursorBounds, Placement placement) {
	...
	/**
	 * Computes a best-fit bounds of popup panel according to available space in
	 * the underlying view (visible part of the pane). The placement is first
	 * evaluated and put into the popup's client property by <CODE>popup.putClientProperty(Placement.class,
	 * actual-placement)</CODE>. The actual placement is
	 * <UL>
	 * <LI> <CODE>Above</CODE> if the original placement was <CODE>Above</CODE>.
	 * Or if the original placement was <CODE>AbovePreferred</CODE> or <CODE>Largest</CODE>
	 * and there is more space above the cursor than below it.
	 * <LI> <CODE>Below</CODE> if the original placement was <CODE>Below</CODE>.
	 * Or if the original placement was <CODE>BelowPreferred</CODE> or <CODE>Largest</CODE>
	 * and there is more space below the cursor than above it.
	 * <LI> <CODE>AbovePreferred</CODE> if the original placement was <CODE>AbovePreferred</CODE>
	 * and there is less space above the cursor than below it.
	 * <LI> <CODE>BelowPreferred</CODE> if the original placement was <CODE>BelowPreferred</CODE>
	 * and there is less space below the cursor than above it.
	 * <P>
	 * Once the placement client property is set the <CODE>popup.setSize()</CODE>
	 * is called with the size of the area above/below the cursor (indicated by
	 * the placement). The popup responds by updating its size to the equal or
	 * smaller size. If it cannot physically fit into the requested area it can
	 * call <CODE>putClientProperty(Placement.class, null)</CODE> on itself to
	 * indicate that it cannot fit. The method scans the content of the client
	 * property upon return from <CODE>popup.setSize()</CODE> and if it finds
	 * null there it returns null bounds in that case. The only exception is if
	 * the placement was either <CODE>AbovePreferred</CODE> or <CODE>BelowPreferred</CODE>.
	 * In that case the method gives it one more try by attempting to fit the
	 * popup into (bigger) complementary <CODE>Below</CODE> and <CODE>Above</CODE>
	 * areas (respectively). The popup either fits into these (bigger) areas or
	 * it again responds by returning <CODE>null</CODE> in the client property
	 * in which case the method finally gives up and returns null bounds.
	 * 
	 * @param popup
	 *            popup panel to be displayed
	 * @param viewWidth
	 *            width of the visible view area.
	 * @param viewHeight
	 *            height of the visible view area.
	 * @param cursorBounds
	 *            the bounds of the caret or mouse cursor relative to the
	 *            upper-left corner of the visible view
	 * @param placement
	 ...
	 *            position
	 * @return bounds of popup panel relative to the upper-left corner of the
	 *         underlying view. <CODE>null</CODE> if there is no place to
	 *         display popup.
	 */
	protected static Rectangle computeBounds(JComponent popup, int viewWidth,
			int viewHeight, Rectangle cursorBounds, Placement placement) {
		...
		while (true) { // do one or two passes
			...
			int height = (placement == Above || placement == AbovePreferred) ? aboveCursorHeight
					: belowCursorHeight;

			popup.setSize(viewWidth, height);
			popupBounds = popup.getBounds();

			Placement updatedPlacement = (Placement) popup
					.getClientProperty(Placement.class);

			if (updatedPlacement != placement) { // popup does not fit with
				// the orig placement
		...
		if (popupBounds != null) {
			...
			popupBounds.x = Math.min(cursorBounds.x, viewWidth
					- popupBounds.width);

			popupBounds.y = (placement == Above || placement == AbovePreferred) ? (aboveCursorHeight - popupBounds.height)
					: belowCursorY;
		}
